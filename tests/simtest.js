const fs=require('fs');
const code=['data','engine','ai'].map(f=>fs.readFileSync(require('path').resolve(__dirname,'../src/'+f+'.js'),'utf8')).join('\n');
...
play(R,[0,1,0,1],99);
play(R,[0,1,2,3],7);
{ const g=play(['hum','orc'],[0,1],5); console.log('outposts', JSON.stringify(g.outposts.map(o=>o.owner)), 'power', g.players.map(p=>Math.round(p.power)+' cd'+p.pcd.map(Math.round))); }
