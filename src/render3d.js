...
const PAT3 = { stone: [1, 24], roof: [2, 18], planks: [3, 21], plaster: [4, 40], thatch: [5, 16], rock: [6, 55], wood: [7, 22], leaf: [8, 14], mail: [8, 4], cloth: [8, 7], fur: [7, 8], leather: [8, 10], metal: [8, 26], skin: [8, 14] };
...
function treePrims(v) {
  ...
  if (conifer) {
    ...
    const base = autumn ? (v === 6 ? [[92, 36, 16], [224, 145, 58]] : [[74, 53, 16], [216, 180, 74]]) : v % 3 === 1 ? [[29, 58, 28], [110, 154, 69]] : [[39, 64, 31], [138, 166, 80]];
    for (let k = 0; k < 11; k++) {
      const a = r() * 6.283, d = Math.sqrt(r()) * 17, y = 50 + (r() - 0.3) * 20, rad = 10 + r() * 7;
      const light = clamp(0.35 + (y - 45) / 40 + (r() - 0.5) * 0.3, 0, 1);
      P.push(sph(Math.cos(a) * d, y, Math.sin(a) * d * 0.9, rad, MAT(mix3(base[0], base[1], light).map(c => c / 255), { pat: 'leaf' })));
    }
  }
  return P;
}
...
class Renderer3D extends Renderer {
  ...
  buildTerrain3() {
    ...
    mat.onBeforeCompile = sh => {
      Object.assign(sh.uniforms, { tMask: { value: mskT }, tDG: { value: dG }, tDD: { value: dD }, tDR: { value: dR }, tTr: { value: this.trampleT3 }, tCl: { value: cl }, uTime, uCloud, uEdge: { value: edge }, uMap: { value: new T.Vector2(MAP_W, MAP_H) }, uSnow: { value: b === 'snow' ? 1 : 0 } });
      sh.vertexShader = 'varying vec3 vWP;\nvarying vec3 vWN;\n' + sh.vertexShader.replace('#include <begin_vertex>', '#include <begin_vertex>\nvWP = (modelMatrix * vec4(transformed, 1.0)).xyz; vWN = normalize(mat3(modelMatrix) * objectNormal);');
      sh.fragmentShader = 'varying vec3 vWP;\nvarying vec3 vWN;\nuniform sampler2D tMask, tDG, tDD, tDR, tTr, tCl;\nuniform float uTime, uCloud, uSnow;\nuniform vec3 uEdge;\nuniform vec2 uMap;\n' + sh.fragmentShader.replace('#include <map_fragment>', `#include <map_fragment>
        ...
        diffuseColor.rgb *= mix(vec3(1.0), vec3(det * 2.0), dA * (1.0 - 0.35 * mk.b / tot));
        diffuseColor.rgb *= 1.0 - 0.38 * clamp(mk.b * 1.4 - 0.2, 0.0, 1.0) * (1.0 - uSnow * 0.5);
        float fine = texture2D(tDG, vWP.xz / 23.0).r;
        diffuseColor.rgb *= 0.9 + fine * 0.2;
        ...
        float slope = 1.0 - clamp((vWN.y - 0.55) * 3.0, 0.0, 1.0);
        vec3 outer = mix(uEdge * (0.7 + det * 0.6), vec3(0.23, 0.22, 0.21) * (0.6 + rr * 0.8), clamp(slope + (vWP.y - 120.0) / 200.0, 0.0, 1.0));
        if (uSnow > 0.5) outer = mix(outer, vec3(0.86, 0.89, 0.93), clamp((vWP.y - 60.0) / 120.0, 0.0, 0.85));
        diffuseColor.rgb = mix(diffuseColor.rgb, outer, smoothstep(60.0, 520.0, outD));
        float cloud = texture2D(tCl, vWP.xz / 1400.0 + vec2(uTime * 0.01, uTime * 0.005)).a;
        diffuseColor.rgb *= 1.0 - cloud * uCloud;`);
    };
  ...
  bldGeo(d, col) {
    ...
    P.push(box(0, -14, 0, d.r * 0.98, 14.5, d.r * 0.78, MAT(RACES[d.race] ? RACES[d.race].stone : '#777', { pat: 'stone' })));
